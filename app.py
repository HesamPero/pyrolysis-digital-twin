...
PRESETS = {
    "Custom":              None,
    "Wood (Pine)":         (54.2,  9.4, 23.2, 0.4, 500),
    "Wheat Straw":         (38.0, 29.0, 15.0, 7.0, 550),
    "Rice Husk":           (31.0, 24.0, 12.0,20.0, 600),
    "Corn Stover":         (40.0, 25.0, 18.0, 5.0, 500),
    "Sugarcane Bagasse":   (41.0, 28.0, 21.0, 2.0, 550),
    "Miscanthus":          (43.0, 24.0, 22.0, 3.0, 600),
}
...
def run_model(cel, hemi, lig, ash, HTT):
    """
    Generalised pyrolysis correlations for demonstration.
    Based on published literature trends (Neves et al. 2011, Antal & Grønli 2003).
...
with st.sidebar:
    ...
    preset = st.selectbox("🌱 Biomass Preset", list(PRESETS.keys()))

    # defaults from preset
    if PRESETS[preset]:
        d_cel, d_hemi, d_lig, d_ash, d_HTT = PRESETS[preset]
    else:
        d_cel, d_hemi, d_lig, d_ash, d_HTT = 40.0, 20.0, 25.0, 5.0, 550

    st.markdown("---")
    st.markdown("**Biochemical Composition [%]**")

    cel  = st.slider("🌿 Cellulose",      20.0, 60.0, float(d_cel),  0.1)
    hemi = st.slider("🍂 Hemicellulose",   5.0, 35.0, float(d_hemi), 0.1)
    lig  = st.slider("🪵 Lignin",          5.0, 60.0, float(d_lig),  0.1)
    ash  = st.slider("⚪ Ash",             0.0, 10.0, float(d_ash),  0.1)
    ...
    HTT = st.slider("🌡️ HTT [°C]", 450, 850, int(d_HTT), 10)
...
st.markdown(
    ...
    "🌿 Biomass Pyrolysis Digital Twin · Public Demo · Built with Python & Streamlit<br>"
    "Physics model uses generalised literature correlations (Neves et al. 2011, "
    "Antal &amp; Grønli 2003). Proprietary experimental data not included."
    "</div>",
    unsafe_allow_html=True,
)
